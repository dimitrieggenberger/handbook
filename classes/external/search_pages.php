<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

namespace local_handbook\external;

use context_system;
use core_external\external_api;
use core_external\external_function_parameters;
use core_external\external_multiple_structure;
use core_external\external_value;

/**
 * External function: search published pages.
 *
 * Matches title, summary and the normalized plain text of the published
 * revision. Only published, non-archived, non-excluded pages are searched
 * (specification 13.1, 17.4).
 *
 * @package   local_handbook
 * @copyright Educación Helvética SA / EuropaSchule
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class search_pages extends external_api {

    /**
     * Parameter definition.
     *
     * @return external_function_parameters
     */
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            'query' => new external_value(PARAM_RAW, 'Search text (min 2 characters)'),
            'contenttype' => new external_value(PARAM_ALPHANUMEXT, 'Filter by content type key',
                VALUE_DEFAULT, ''),
            'categoryid' => new external_value(PARAM_INT, 'Filter by category id (0 = all)',
                VALUE_DEFAULT, 0),
            'page' => new external_value(PARAM_INT, 'Zero-based page number', VALUE_DEFAULT, 0),
            'perpage' => new external_value(PARAM_INT, 'Page size (max 200)', VALUE_DEFAULT, 20),
        ]);
    }

    /**
     * Search pages.
     *
     * @param string $query Search text.
     * @param string $contenttype Content type filter.
     * @param int $categoryid Category filter.
     * @param int $page Zero-based page number.
     * @param int $perpage Page size.
     * @return array
     */
    public static function execute(string $query, string $contenttype = '', int $categoryid = 0,
            int $page = 0, int $perpage = 20): array {
        global $DB;

        $params = self::validate_parameters(self::execute_parameters(), [
            'query' => $query,
            'contenttype' => $contenttype,
            'categoryid' => $categoryid,
            'page' => $page,
            'perpage' => $perpage,
        ]);

        $context = context_system::instance();
        self::validate_context($context);
        helper::require_read($context);

        $query = trim($params['query']);
        if (\core_text::strlen($query) < 2) {
            return [];
        }

        [$limitfrom, $limitnum] = helper::paginate($params['page'], $params['perpage']);

        $like = '%' . $DB->sql_like_escape($query) . '%';
        $where = "p.aiaccess <> 'excluded' AND p.archived = 0 AND p.publishedrevisionid > 0 AND ("
            . $DB->sql_like('p.title', ':q1', false) . ' OR '
            . $DB->sql_like('p.summary', ':q2', false) . ' OR '
            . $DB->sql_like('r.plaintext', ':q3', false) . ')';
        $sqlparams = ['q1' => $like, 'q2' => $like, 'q3' => $like];

        if ($params['contenttype'] !== '') {
            $where .= ' AND p.contenttype = :contenttype';
            $sqlparams['contenttype'] = $params['contenttype'];
        }
        if ($params['categoryid']) {
            $where .= ' AND p.categoryid = :categoryid';
            $sqlparams['categoryid'] = $params['categoryid'];
        }

        $sql = "SELECT p.*, r.versionnumber AS pubversion, r.timepublished AS pubtime,
                       r.contenthash AS pubhash
                  FROM {local_handbook_page} p
                  JOIN {local_handbook_revision} r ON r.id = p.publishedrevisionid
                 WHERE $where
              ORDER BY p.title ASC";

        $result = [];
        foreach ($DB->get_records_sql($sql, $sqlparams, $limitfrom, $limitnum) as $record) {
            $result[] = helper::export_page_summary($record);
        }
        return $result;
    }

    /**
     * Return definition.
     *
     * @return external_multiple_structure
     */
    public static function execute_returns(): external_multiple_structure {
        return new external_multiple_structure(helper::page_summary_structure());
    }
}
