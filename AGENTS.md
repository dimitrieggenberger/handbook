...
- `mcp/`: standalone MCP adapter (spec §18) — Node, not shipped in the ZIP, tokens via env vars only; shared tool/REST logic in `mcp/lib/handbook.mjs`, stdio entry `mcp/server.mjs`, `HANDBOOK_MCP_MODE` = readwrite-drafts|readonly. Sibling WordPress adapter (`mcp/lib/wordpress.mjs`, stdio `mcp/wordpress-server.mjs`, `WORDPRESS_*` env vars) for the public site with the same drafts-only guarantee; handbook↔website consistency workflow in `docs/WORDPRESS_HARMONIZATION.md`; `mcp/http-server.mjs` serves both toolsets on one endpoint when the WordPress secrets are set
